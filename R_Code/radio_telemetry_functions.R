...
#we do not need this function any more. It's replacement is located below
...
#we do not have to do this any more as the 434MHz tags give us dbm for free. 
...
xi_square_calc_dbm_r_psi <- function(r, psi, z, ant_HT, min_xi_dbm, lambda, D0, p0){
  ...
  # psi <- angle in radians around antenna
   <- cos(pi/2*sin(psi))/cos(psi) * (sin(p+q*cos(psi))/(p+q*cos(psi))) #version from Janaswamy - eqn 19 - Antenna field pattern
